...
def process_string(input_strings):
    ...
    if 30 <= len(input_strings) <= 50:
        letter_count = sum(c.isalpha() for c in input_strings)
        digit_count = sum(c.isdigit() for c in input_strings)
        return f"Довжина рядка: {len(input_strings)}, Кількість букв: {letter_count}, Кількість цифр: {digit_count}"
    elif len(input_strings) < 30:
        ...
        digits = [_ for _ in input_strings if _.isdigit()]
        letters = ''.join(c for c in input_strings if c.isalpha() and not c.isspace())
