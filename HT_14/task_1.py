...
class ATM_USER:
    ...
    def take_current_exchange(self,today = None):
        today = datetime.date.today()
        url = f'https://api.privatbank.ua/p24api/exchange_rates?date={today.strftime("%d.%m.%Y")}'

        response = requests.get(url)
        if response.status_code == 200:
            data = response.json()
            data = self.check_exchange(data)
            final_text = [f"Дата поточного курсу {data['date']}"]
            currency = ["USD","PLN","EUR"]
            for dt in data["exchangeRate"]:
                if "saleRate" in dt:
                    if dt['currency'] in currency:
                        final_text.append(f"{dt['currency']}:\nПродаж: {dt['saleRate']}\nКупівля: {dt['purchaseRate']}\n{'-'*20}")
            return '\n'.join(final_text)   

        else:
            return "Помилка спробуйте знову"
    
    def check_exchange(self,data):
        if len(data["exchangeRate"]) == 0:
            yesterday = datetime.date.today() - datetime.timedelta(days=1)
            url = f'https://api.privatbank.ua/p24api/exchange_rates?date={yesterday.strftime("%d.%m.%Y")}'
            response = requests.get(url)
            if response.status_code == 200:
                data = response.json()
                return data
...
def user_panel(atm):
    user_action = input("Оберіть дію: \nзняти кошти(1)\nположити кошти(2)\n"
                        "переглянути рахунок(3)\nдля виходу(4)\nдля того щоб дізнатись поточний курс валют(5)\n"
                        "напишіть цифру дії яку бажаєте обрати: ")
    if user_action == "1":
        ...
        print(atm.take_current_exchange())
    else:
...
def admin_panel(atm_admin):
    admin_action = input("Оберіть дію: \nзабрати купюри(1)\nположити купюри(2)\n"
                         "\nпереглянути купюри(3)\nдля виходу(4)\n"
                         "напишіть цифру дії яку бажаєте обрати: ")
    if admin_action == "1":
...
def start():
    ...
    atm = ATM_USER()
    atm_admin = ATM_ADMIN()
    start_quest = input("Для того щоб зайти в аккаунт(1)\nдля того щоб зареєструватись (2)\nдля виходу (3): ")
    if start_quest == "3":
        print("Гарного дня")
        exit()
